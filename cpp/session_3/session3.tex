...
\frame
{
  ...
  \item Ne peut être copié mais seulement transféré
...
\frame
{
  \frametitle{\texttt{std::shared\_ptr}}
  \only<1>{
  \begin{block}{Principes}
  \begin{itemize}
  \item Pointeur à compteur de références
  \item Libère la mémoire lorsque aucune référence ne pointe sur lui
  \item Cycles gérés par \texttt{std::weak\_ptr}
  \item Conversion de \texttt{this} avec \texttt{std::enable\_shared\_from\_this}
  \end{itemize}
  \end{block}
  }
  \only<2>{\sourcecodecpp{code/raii/shared_ptr.cpp}}
  \only<3>{\sourcecodecpp{code/raii/weak_ptr.cpp}}
  \only<4>{\sourcecodecpp{code/raii/shared_from_this.cpp}}
}
